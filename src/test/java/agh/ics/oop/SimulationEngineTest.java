...
public class SimulationEngineTest {
    ...
    void runWithOneAnimal() {
        MoveDirection[] directions = OptionParser.parse(new String[] {"f","l"});
    ...
    void runWithTwoAnimals() {
        MoveDirection[] directions = OptionParser.parse(new String[] {"f", "b", "r", "l", "f", "f", "r", "r", "f", "f", "f", "f", "f", "f", "f", "f"});
        RectangularMap map = new RectangularMap(10, 5);
        Vector2d[] positions = {new Vector2d(2,2), new Vector2d(3,4) };
        IEngine engine = new SimulationEngine(directions, map, positions);
        engine.run();
        List<Animal> animals = map.getAnimals();
        var animal1ExpectedPosition = new Vector2d(3, 4);
        var animal2ExpectedPosition = new Vector2d(2, 0);
        var animal1ExpectedDirection = MapDirection.NORTH;
        var animal2ExpectedDirection = MapDirection.SOUTH;
